//! Zeta Test World - "Test as World" Implementation
//!
//! This module implements a "Test World" where the game state is the
//! spectral analysis of a graph. It serves two purposes:
//! 1. A rigorous unit test for Ihara-Hashimoto spectral gap calculations.
...
test "zeta world initialization and tick" {
    const allocator = std.testing.allocator;
    
    var world = try ZetaWorld.init(allocator, 10);
    defer world.deinit();
    
    try std.testing.expectEqual(@as(usize, 10), world.n);
    ...
    // Assert spectral properties
    // Cycle graph C_10 (k=2 regular) -> lambda1 = 2
    // With mutation, it might change, but should be valid float
    try std.testing.expect(world.eigenvalues[0] >= 0);
    try std.testing.expect(world.spectral_gap >= 0);
    
    // Assert entropy exists
    try std.testing.expect(world.entropy >= 0);
    
    std.debug.print("\nZeta World Test Passed! Gap: {d:.4}, Entropy: {d:.4}\n", .{world.spectral_gap, world.entropy});
}
