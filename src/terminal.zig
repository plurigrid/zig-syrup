...
pub const Color = packed struct(u32) {
    ...
    pub fn toRgb24(self: Color) u24 {
        switch (self.tag) {
            ...
            .palette => {
                const idx: u8 = @intCast(self.payload & 0xFF);
                if (idx < 16) {
                    // Standard + bright ANSI colors
                    const ansi16 = [16]u24{
                        0x000000, 0xAA0000, 0x00AA00, 0xAA5500, // black, red, green, yellow
                    ...
                    const cube = idx - 16;
                    const ri: u24 = cube / 36;
                    const gi: u24 = (cube % 36) / 6;
                    const bi: u24 = cube % 6;
                    const steps = [6]u8{ 0, 95, 135, 175, 215, 255 };
                    return (@as(u24, steps[ri]) << 16) | (@as(u24, steps[gi]) << 8) | @as(u24, steps[bi]);
                } else {
                    // Grayscale ramp (indices 232-255)
                    const g: u24 = 8 + @as(u24, idx - 232) * 10;
                    return (g << 16) | (g << 8) | g;
                }
            },
...
test "cell pack and unpack roundtrip" {
    const cell = Cell{ .codepoint = 0x1F600, .fg = Color.rgb(0xAB, 0xCD, 0xEF), .bg = Color.rgb(0x12, 0x34, 0x56), .attrs = .{ .bold = true, .italic = true } };
...
test "dispatch frame/sync" {
    var grid = Grid.init(80, 24);
    grid.put(0, 0, 'Z', Color.rgb(0xAB, 0xCD, 0xEF), Color.BLACK, .{});
...
test "dispatch cell/read" {
    var grid = Grid.init(80, 24);
    const fg = Color.rgb(0x11, 0x22, 0x33);
    const bg = Color.rgb(0x44, 0x55, 0x66);
    grid.put(3, 7, 'Q', fg, bg, .{ .bold = true });
    ...
    try testing.expectEqual(fg, result.cell.?.fg);
}
...
test "full pipeline: write → commit → decode → apply" {
    // Source terminal — GF(3) trit primaries: PLUS(red) ERGODIC(green) MINUS(blue)
