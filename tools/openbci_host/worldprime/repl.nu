#!/usr/bin/env nu
# repl.nu - Interactive REPL for worldprime
# Custom prompt, command history, tab completion, persistent variables

use themes.nu *

const HISTORY_FILE = ($nu.home-path | path join ".config" "worldprime" "repl_history.txt")
const VARS_FILE = ($nu.home-path | path join ".config" "worldprime" "repl_vars.nuon")
const WELCOME_BANNER = "
╔══════════════════════════════════════════════════════════════════╗
║                                                                  ║
║   🧠 worldprime Interactive REPL                                   ║
║                                                                  ║
...
def show-worlds []: [ nothing -> string ] {
    let worlds_dir = ($nu.home-path | path join ".config" "worldprime" "worlds")
    
    if not ($worlds_dir | path exists) {
        return "No worlds directory found. Run 'worldprime init' first."
    }
...
def show-sessions []: [ nothing -> string ] {
    let sessions_dir = ($nu.home-path | path join ".config" "worldprime" "sessions")
...
def save-session [state: record, name: string]: [ nothing -> string ] {
    let sessions_dir = ($nu.home-path | path join ".config" "worldprime" "repl_sessions")
    mkdir $sessions_dir
