...
const PIPES_DIR = "/tmp/worldprime"
...
export def main []: [ nothing -> record ] {
    print ""
    print "╔══════════════════════════════════════════════════════════════╗"
    print "║                worldprime Initialization                       ║"
    ...
    print "\n🚀 Quick start:"
    print "   worldprime demo        # Run interactive demo"
...
def generate-shell-configs []: [ nothing -> nothing ] {
    let nu_config = $"# worldprime shell integration
# Add to your config.nu:
#   source ($CONFIG_DIR | path join "shell_integration.nu")

# Module path
export-env {
    $env.NUWORLDS_DIR = "/Users/bob/i/zig-syrup/tools/openbci_host/worldprime"
...
# Completions
use ($env.NUWORLDS_DIR | path join "worldprime.nu") *
"
...
export def reset []: [ nothing -> nothing ] {
    print "⚠️  This will delete all worldprime configuration and data!"
    ...
    if $confirm == "yes" {
        rm -rf $CONFIG_DIR
        rm -rf $DATA_DIR
        print "✓ worldprime configuration reset"
    } else {
        print "Cancelled"
    }
}
