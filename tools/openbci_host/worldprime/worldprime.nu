#!/usr/bin/env nu
# worldprime.nu - Master command for worldprime
# Single entry point: worldprime <command>
...
const VERSION = "0.2.0"
const CONFIG_DIR = ($nu.home-path | path join ".config" "worldprime")

# =============================================================================
# Main Entry Point
# =============================================================================

# worldprime - OpenBCI + A/B World Testing Framework
#
# Usage:
#   worldprime demo           # Run interactive demo
#   worldprime workflow       # Run pre-built workflows
#   worldprime dashboard      # Launch real-time dashboard
#   worldprime repl           # Start interactive REPL
#   worldprime doctor         # System diagnostics
#   worldprime update         # Check for updates
#
# Examples:
#   worldprime demo --mode full
#   worldprime workflow bci-focus-tracker
#   worldprime dashboard --theme matrix
export def main []: [ nothing -> string ] {
    $"worldprime v($VERSION) - OpenBCI + A/B World Testing Framework

USAGE:
    worldprime <command> [args]
...
COMMANDS:
    ...
    init        Initialize worldprime (first run)
...
EXAMPLES:
    ...
    worldprime dashboard --theme ocean
...
export def "main demo" [
    ...
    if not (is-initialized) {
        print "⚠️  worldprime not initialized. Running init first..."
        init
        print ""
    }
    
    let demo_script = ($env.FILE_PWD? | default "/Users/bob/i/zig-syrup/tools/openbci_host/worldprime" | path join "demo.nu")
...
export def "main workflow" [
    ...
    let workflows_script = ($env.FILE_PWD? | default "/Users/bob/i/zig-syrup/tools/openbci_host/worldprime" | path join "workflows.nu")
    ...
    if $list or $name == null {
        ...
        print ""
        print "Usage: worldprime workflow <name>"
        return
    }
...
export def "main dashboard" [
    ...
    let dashboard_script = ($env.FILE_PWD? | default "/Users/bob/i/zig-syrup/tools/openbci_host/worldprime" | path join "dashboard.nu")
...
export def "main repl" [
    ...
    let repl_script = ($env.FILE_PWD? | default "/Users/bob/i/zig-syrup/tools/openbci_host/worldprime" | path join "repl.nu")
...
export def "main doctor" []: [ nothing -> record ] {
    print "🔬 worldprime System Diagnostics\n"
    ...
    if $results.initialized {
        print "  ✓ worldprime initialized"
    } else {
        print "  ✗ worldprime not initialized"
        $results.issues = ($results.issues | append "Not initialized")
        $results.recommendations = ($results.recommendations | append "Run: worldprime init")
    }
    ...
    let dirs = {
        ...
        recordings: (($nu.home-path | path join ".local" "share" "worldprime" "recordings") | path exists)
    }
...
export def "main theme" [
    ...
    if $action == null or $action == "list" {
        ...
        if $name == null {
            print "Usage: worldprime theme set <name>"
        ...
        print "Usage: worldprime theme [list|set|demo]"
    }
}
...
def run-builtin-demo [mode: string]: [ nothing -> nothing ] {
    print $"🧠 worldprime Demo (mode: ($mode))\n"
...
def run-builtin-repl []: [ nothing -> nothing ] {
    print "🖥️  worldprime REPL (built-in)"
