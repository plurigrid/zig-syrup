#!/usr/bin/env nu
# utils.nu - Utility functions for worldprime
...
export def auto-config [
    ...
    # Save config
    let config_file = ($nu.home-path | path join ".config" "worldprime" "auto_config.nuon")
    {
        device: $device_info
        config: $config
        impedance: $impedance
...
export def validate-setup [--fix]: [ nothing -> record ] {
    ...
    print "🔬 Validating worldprime setup...\n"
    ...
    let config_dir = ($nu.home-path | path join ".config" "worldprime")
    let required_dirs = [
        $config_dir
