#!/usr/bin/env nu
# aliases.nu - Short command aliases for worldprime
...
# nw - worldprime
export alias nw = worldprime

# nwd - worldprime demo
export alias nwd = worldprime demo

# nwdf - worldprime demo --mode full
export alias nwdf = worldprime demo --mode full

# nwdb - worldprime demo --mode bci-only
export alias nwdb = worldprime demo --mode bci-only

# nww - worldprime workflow
export alias nww = worldprime workflow

# nwdash - worldprime dashboard
export alias nwdash = worldprime dashboard

# nwr - worldprime repl
export alias nwr = worldprime repl

# nwdoc - worldprime doctor
export alias nwdoc = worldprime doctor
...
export alias focus = worldprime workflow bci-focus-tracker

# Quick meditation session
export alias meditate = worldprime workflow meditation-monitor

# Quick A/B test
export alias abtest = worldprime workflow ab-test-eeg

# Quick game
export alias game = worldprime workflow neurofeedback-game

# Quick sleep recording
export alias sleeprec = worldprime workflow sleep-recorder

# =============================================================================
# Directory Shortcuts
# =============================================================================

# cd to worldprime config
export def cdnw []: [ nothing -> nothing ] {
    cd ($nu.home-path | path join ".config" "worldprime")
}

# cd to worldprime recordings
export def cdnwr []: [ nothing -> nothing ] {
    cd ($nu.home-path | path join ".local" "share" "worldprime" "recordings")
}

# cd to worldprime source
export def cdnws []: [ nothing -> nothing ] {
    cd "/Users/bob/i/zig-syrup/tools/openbci_host/worldprime"
}

# List recordings
export def lsr []: [ nothing -> table ] {
    ls ($nu.home-path | path join ".local" "share" "worldprime" "recordings")
}

# List worlds
export def lsw []: [ nothing -> table ] {
    ls ($nu.home-path | path join ".config" "worldprime" "worlds")
}

# List sessions
export def lss []: [ nothing -> table ] {
    ls ($nu.home-path | path join ".config" "worldprime" "sessions")
}
...
# theme list
export alias tl = worldprime theme list

# theme set default
export alias td = worldprime theme set default

# theme set ocean
export alias to = worldprime theme set ocean

# theme set matrix
export alias tm = worldprime theme set matrix
...
export def status []: [ nothing -> record ] {
    {
        initialized: ($nu.home-path | path join ".config" "worldprime" "initialized.nuon" | path exists)
        config_dir: ($nu.home-path | path join ".config" "worldprime") | path exists
        recordings_dir: ($nu.home-path | path join ".local" "share" "worldprime" "recordings") | path exists
...
export def h []: [ nothing -> nothing ] {
    print "worldprime Quick Reference:"
    print ""
    print "OpenBCI:"
