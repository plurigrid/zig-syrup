# themes.nu
# Color themes for worldprime visualizations
...
export def current-theme []: [ nothing -> record ] {
    let config_file = ($nu.home-path | path join ".config" "worldprime" "config.nuon")
...
export def set-theme [name: string]: [ nothing -> nothing ] {
    ...
    if not ($config_file | path exists) {
        error make { msg: "worldprime not initialized. Run 'init' first." }
    }
