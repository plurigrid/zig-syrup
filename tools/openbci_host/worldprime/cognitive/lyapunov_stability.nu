...
# Lyapunov Stability Analysis for worldprime cognitive control
    ...
    if $dim == 2 {
        ...
        for i in 0..<$resolution {
            ...
            for j in 0..<$resolution {
                ...
                # Simulate trajectory
                let sim_result = (simulate-dynamics $dynamics $initial $max_time)
                let final_point = ($sim_result.trajectory | last)
...
# Class D fix: dt-aware simulation with adaptive step and error reporting.
# Estimates Lipschitz constant from the dynamics to bound Euler error.
# Returns record with trajectory and diagnostics instead of bare list.
def simulate-dynamics [dynamics: closure, initial: list, steps: int, --dt: float = 0.01, --max-val: float = 100.0]: [ nothing -> record ] {
    mut trajectory = [$initial]
    mut current = $initial
    mut max_derivative_norm = 0.0
    mut step_count = 0
    mut diverged = false

    for _ in 0..<$steps {
        let derivative = (do $dynamics $current)
        let deriv_norm = ($derivative | each {|x| $x | math abs} | math max)
        if $deriv_norm > $max_derivative_norm { $max_derivative_norm = $deriv_norm }

        # CFL-like check: dt * |f'| < 1 for Euler stability
        let cfl = $dt * $deriv_norm
        let effective_dt = if $cfl > 0.5 {
            0.5 / (if $deriv_norm > 1e-12 { $deriv_norm } else { 1e-12 })
        } else {
            $dt
        }

        $current = ($current | zip $derivative | each {|p| $p.0 + ($p.1 * $effective_dt)})
        $trajectory = ($trajectory | append $current)
        $step_count = $step_count + 1

        if ($current | each {|x| $x | math abs} | math max) > $max_val {
            $diverged = true
            break
        }
    }

    # Euler global error bound: O(dt * L * T) where L = max |f'|, T = steps * dt
    let error_bound = $dt * $max_derivative_norm * $step_count * $dt / 2.0
